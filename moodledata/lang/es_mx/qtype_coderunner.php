...
$string['allowedfilenames_help'] = 'Todos los nombres subidos deben coincidir con la expresión regular de PHP (Perl) dada, si no estuvieran vacíos. Por ejemplo, use \'.+\\\\.cpp\' para  permitir cualquier archivo C++ o \'(?!Prog)\\\\.java\' para permitir cualquier archivo Java excepto \'Prog.java\'. Adicionalmente, los nombres de archivos deben contener solamente caracteres alfanuméricos más guion bajo, guion normal y punto; no deben comenzar con doble subrayado (\'\\_\\_\') y no deben tener conflictos con ninguno de los nombres de archivos de soporte. La Description es un mensaje de texto mostrado al estudiante para explicarle cual(es) archivos son esperados. Dejar vacío para mostrar la expresión regular misma. Dejar ambos vacíos para sobrepasar la revisión de expresión regular.';
...
$string['badjson'] = 'Salida JSON mala de la salida del calificador combinador. La salida era: {$a->output}';
