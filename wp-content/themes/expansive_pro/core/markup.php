		...
		if(is_array($links)){ 
			foreach($links as $current_link){
				if($current_link['url'] != ''){
					...
					if($current_link['icon'] != '') echo cpotheme_icon($current_link['icon'], 'social-icon');
				...
				if($link_url != ''){
					foreach($social_networks as $current_network_id => $current_network_data){
						if(strpos($link_url, $current_network_id) !== false){
							...
							cpotheme_icon($current_network_data['icon'], 'social-icon');
...
if(!function_exists('cpotheme_slider_data')){	
	function cpotheme_slider_data($navigation = true, $pagination = true){ 
		...
		$slider_effect = cpotheme_get_option('slider_effect');
...
if(!function_exists('cpotheme_section_heading')){
	function cpotheme_section_heading($slug){
		$slug = esc_attr($slug);
		$heading = cpotheme_get_option('home_'.$slug);
		if($heading){
			echo '<div class="section-heading '.$slug.'-heading">';
			echo '<div class="section-title '.$slug.'-title heading">';
			echo $heading;
			echo '</div>';
			cpotheme_block('home_'.$slug.'_subtitle', $slug.'-subtitle section-subtitle');
			echo '</div>';
		}
	}
}
...
//Print Tagline image
if(!function_exists('cpotheme_tagline_image')){
	function cpotheme_tagline_image(){
		$tagline = cpotheme_get_option('home_tagline_image');
		if($tagline != ''){
