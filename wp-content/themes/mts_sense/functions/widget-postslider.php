	...
	class mts_post_slider_widget extends WP_Widget {
		...
		public function __construct() {
			parent::__construct(
				...
				sprintf( __('%sPost Slider', 'ad-sense' ), MTS_THEME_WHITE_LABEL ? '' : 'MTS ' ),
				array( 'description' => __( 'Display posts from multiple categories in an animated slider.', 'ad-sense' ) )
			);
		}

		public function form( $instance ) {
			$defaults = array(
				'title' => __( 'Featured Posts', 'ad-sense' ),
				'cat' => array(),
				'slides_num' => 3,
				'show_title' => 0,
				'slider_nav' => 'bullets',
			...
			<p>
				<label for="<?php echo $this->get_field_id( 'slides_num' ); ?>"><?php _e( 'Number of Posts to show', 'ad-sense' ); ?></label>
			...
			<p>
				<label for="<?php echo $this->get_field_id( 'slider_nav' ); ?>"><?php _e( 'Navigaton type:', 'ad-sense' ); ?></label>
				<select id="<?php echo $this->get_field_id( 'slider_nav' ); ?>" name="<?php echo $this->get_field_name( 'slider_nav' ); ?>" class="widefat">
					<option value="bullets"<?php selected( $instance['slider_nav'], 'bullets' ); ?>><?php _e( 'Bulleted', 'ad-sense' ); ?></option>
					<option value="arrows"<?php selected( $instance['slider_nav'], 'arrows' ); ?>><?php _e( 'Arrows', 'ad-sense' ); ?></option>
				</select>
			</p>
						...
						<div class="widget-slider widget-slider-<?php echo $slider_nav; ?>">
							...
							<div class="slide">
								<a href="<?php echo esc_url( get_the_permalink() ); ?>">
									...
									the_post_thumbnail( 'ad-sense-widgetfull', $attr );
									?>
									<?php if ( $show_title ) { ?>
