...
class MTS_Options_Tab_Support {
	...
	public function add_support_tab( $sections ) {
		...
		$sections['support'] = array(
			...
			sprintf( __( 'We offer technical support through our %s. Please <strong>copy and paste the following information in your ticket</strong> when contacting support:' , 'ad-sense' ), '<a href="https://community.mythemeshop.com/forum/7-theme-support/" target="_blank">' . __( 'Support Ticket System', 'ad-sense' ) . '</a>' )
			 . '</p>',
	...
	public function add_support_tab_content() {
		?>
		<input type="hidden" id="mts_debug_nonce" value="<?php echo wp_create_nonce( 'mts_debug' ); ?>">
		<div class="mts-options-extra-tab-content">
			<?php $this->support_buttons(); ?>

			<textarea class="large-text" id="mts-debug-data-field" rows="16" readonly="readonly" disabled><?php _e( 'Generating log. Please wait...', 'ad-sense' ); ?></textarea>
	...
	public function support_buttons() {
		...
		<div class="mts-help-buttons">
			<button type="button" class="button mts-support-copy" disabled>
				...
				<?php _e( 'Copy Data for Support Request', 'ad-sense' ); ?>
			</button> 
			<a href="https://community.mythemeshop.com/forum/7-theme-support/" target="_blank" class="button button-primary mts-support-link">
				...
				<?php _e( 'Open Support Forum', 'ad-sense' ); ?>
			</a>
		</div>
		<?php
	}
	...
	public function get_debug_data() {
		...
		$data['environment'] = array(
			'title' => __( 'Environment', 'ad-sense' ),
			'data' => $this->get_environment_info(),
		);

		$data['wp'] = array(
			'title' => __( 'WordPress', 'ad-sense' ),
			'data' => $this->get_wp_info(),
		);

		$data['plugins'] = array(
			'title' => __( 'Plugins', 'ad-sense' ),
			'data' => $this->get_plugins_info(),
		);

		$data['themes'] = array(
			'title' => __( 'Theme', 'ad-sense' ),
			'data' => $this->get_theme_info(),
		);

		$data['settings'] = array(
			'title' => __( 'Theme Settings', 'ad-sense' ),
