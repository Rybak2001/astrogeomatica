...
class mtsGoogleTypography {
	...
	function help_tab() {
		...
		$adding_title					= __('Adding A Collection', 'ad-sense' );
		$adding_content				= '<p>'.__('To add a new font for use on your site. Click the "Add New" button on the top left of the page near the "Google Typography" title.', 'ad-sense' ).'</p>';
		$adding_content			 .= '<p>'.__('Once added, a new font row will appear on the page below. Next you can continue to customize your font (more info in the "Customizing" help tab).', 'ad-sense' ).'</p>';
		//$adding_content			 .= '<p><a href="https://vimeo.com/67957799" target="_blank">'.__('Watch The Video Tutorial &rarr;', 'ad-sense' ).'</a></p>';
		$customizing_title 		= __('Customizing', 'ad-sense' );
		$customizing_content	= '<p>'.__('Customizing fonts is easy; after adding a new font row you can then customize the following font attributes:', 'ad-sense' ).'</p>';
		$customizing_content .= '<ul><li><b>'.__('Preview Text', 'ad-sense' ).'</b> - '.__('Used for live previewing your changes. This text does not appear anywhere on your website.', 'ad-sense' ).'</li><li><b>Preview Background Color</b> - Allows you to swap between light and dark backgrounds when previewing this font.</li><li><b>'.__('Font Family', 'ad-sense' ).'</b> - '.__('The font family to use for this font. Choose from a real-time list of all available Google Fonts.', 'ad-sense' ).'</li><li><b>'.__('Font Variant', 'ad-sense' ).'</b> - '.__('The variant to use for this font. Note: Each font has it\'s own variant options.', 'ad-sense' ).'</li><li><b>'.__('Font Size', 'ad-sense' ).'</b> - '.__('The size you would like this font to be.', 'ad-sense' ).'</li><li><b>'.__('Font Color', 'ad-sense' ).'</b> - '.__('The color you\'d like to use for this font.', 'ad-sense' ).'</li><li><b>'.__('CSS Selectors', 'ad-sense' ).'</b> - '.__('The HTML tags or CSS selectors you\'d like this font to apply to (more info in the "CSS Selectors" help tab). You can specify multiple selectors separated by comma\'s. Ex: h1, #some_id, .some_class', 'ad-sense' ).'</li></ul>';
		$selectors_title 			= __('CSS Selectors', 'ad-sense' );
		$selectors_content		= '<p>' . __('CSS Selectors are used to hook your font rows into your actual website. Once you\'ve added, customized, and defined CSS selectors for your fonts, Google Typography will automatically insert all the necessary CSS into your website.', 'ad-sense' ) . '</p>';
		$selectors_content		= '<p>' . __('Here are some examples of the selectors you can use:', 'ad-sense' ) . '</p>';
		$selectors_content	 .= '<ul><li><b>'.__('IDs', 'ad-sense' ).':</b> '.__('#selector', 'ad-sense' ).'</li><li><b>'.__('Classes:', 'ad-sense' ).':</b> '.__('.selector', 'ad-sense' ).'</li><li><b>'.__('HTML Tags', 'ad-sense' ).':</b> '.__('span', 'ad-sense' ).'</ul>';
		$selectors_content	 .= '<p><b>'.__('Example', 'ad-sense' ).':</b> '.__('#selector span.date', 'ad-sense' ).'</p>';
	...
	function plugin_link($links, $file) {
		if ( $file == 'google-typography/google-typography.php' ) {
			$links['settings'] = sprintf( '<a href="%s"> %s </a>', admin_url( 'themes.php?page=typography' ), __( 'Settings', 'ad-sense' ) );
		}
		return $links;
	}
	...
	function options_ui() {
		...
		$title								= __('Theme Typography', 'ad-sense' );
		$loading							= __('Loading Your Collections', 'ad-sense' );
		$add_new					= __('Add New Collection', 'ad-sense' );
		$reset						= __('Reset Collections', 'ad-sense' );
		$preview_text   			= __('Type in some text to preview...', 'ad-sense' );
		$collection_title 			= __('New Collection', 'ad-sense' );
		//$this->preview_text = $preview_text; // to pass it to JS
		$preview_hint   			= __('Preview Background Color', 'ad-sense' );
		//$font_family_title  	= __('Font family...', 'ad-sense' );
		$std_font_family_title  	= __('Standard Fonts', 'ad-sense' );
		$google_font_family_title  	= __('Google Fonts', 'ad-sense' );
		$font_variant_title 	= __('Variant...', 'ad-sense' );
		$font_size_title 			= __('Size...', 'ad-sense' );
		$backup_font_title		  = __('Fallback:', 'ad-sense' );
		$css_selectors_title	= __('CSS Selectors:', 'ad-sense' );
		$additional_css_title	= __('Additional CSS:', 'ad-sense' );
		$delete_button_text   = __('Delete', 'ad-sense' );
		$save_button_text			= __('Save', 'ad-sense' );
		
		$welcome_title 				= __('Theme Typography', 'ad-sense' );
		$welcome_subtitle			= __('Get started in 3 steps.', 'ad-sense' );//.'<a href="https://vimeo.com/67957799" target="_blank">'.__('Watch the video tutorial &#x2192;', 'ad-sense' ).'</a>';
		$step_1_title 				= __('1. Pick A Font', 'ad-sense' );
		$step_1_desc					= __('Choose from any of the 600+ Google Fonts.', 'ad-sense' );
		$step_2_title 				= __('2. Customize It', 'ad-sense' );
		$step_2_desc					= __('Pick a size, variant, color and more.', 'ad-sense' );
		$step_3_title 				= __('3. Attach It', 'ad-sense' );
		$step_3_desc					= __('Attach your font to any CSS selector(s).', 'ad-sense' );
		$year									= date("Y");
		$themename = MTS_THEME_NAME;
		
		$character_sets_title = __('Character sets', 'ad-sense' );
		$character_sets_desc = __('Choose the character sets you wish to include. Please note that not all sets are available for all fonts.', 'ad-sense' );
		...
		$character_sets = array(
			'latin' => __('Latin', 'ad-sense' ),
			'latin-ext' => __('Latin Extended', 'ad-sense' ),
			'cyrillic' => __('Cyrillic', 'ad-sense' ),
			'cyrillic-ext' => __('Cyrillic Extended', 'ad-sense' ),
			'greek' => __('Greek', 'ad-sense' ),
			'greek-ext' => __('Greek Extended', 'ad-sense' ),
			'vietnamese' => __('Vietnamese', 'ad-sense' ),
			'khmer' => __('Khmer', 'ad-sense' ),
			'devanagari' => __('Devanagari', 'ad-sense' ),
		);
...
EOT;
	...
	function admin_scripts() {
		...
		wp_localize_script('google-typography', 'googletypography', array(
				'reset_confirm' => __("Are you sure you want to revert to the theme's default collections? Note: You will lose any custom collections you've created.", 'ad-sense' ),
				'delete_confirm' => __("Are you sure you want to delete this collection?", 'ad-sense' )
			));
