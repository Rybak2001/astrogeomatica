	...
	*/
	function render(){
		...
		$css_options = array(
			'repeat'	 => array(
				''		  => __('Default', 'ad-sense' ),
				'no-repeat' => __('No Repeat', 'ad-sense' ),
				'repeat'	=> __('Repeat All', 'ad-sense' ),
				'repeat-x'  => __('Repeat Horizontally', 'ad-sense' ),
				'repeat-y'  => __('Repeat Vertically', 'ad-sense' ),
				//'inherit'   => 'Inherit',
			),
			'attachment' => array(
				''		=> __('Default', 'ad-sense' ),
				'fixed'   => __('Fixed', 'ad-sense' ),
				'scroll'  => __('Scroll', 'ad-sense' ),
				//'inherit' => 'Inherit',
			),
			'position'   => array(
				''			  => __('Default', 'ad-sense' ),
				'left top'	  => __('Left Top', 'ad-sense' ),
				'left center'   => __('Left center', 'ad-sense' ),
				'left bottom'   => __('Left Bottom', 'ad-sense' ),
				'center top'	=> __('Center Top', 'ad-sense' ),
				'center center' => __('Center Center', 'ad-sense' ),
				'center bottom' => __('Center Bottom', 'ad-sense' ),
				'right top'	 => __('Right Top', 'ad-sense' ),
				'right center'  => __('Right center', 'ad-sense' ),
				'right bottom'  => __('Right Bottom', 'ad-sense' ),
			),
			'size'	   => array(
				''		=> __('Default', 'ad-sense' ),
				//'inherit' => 'Inherit',
				'cover'   => __('Cover', 'ad-sense' ),
				'contain' => __('Contain', 'ad-sense' ),
			),
			'parallax'   => array(
				'0' => __('Off', 'ad-sense' ),
				'1' => __('On', 'ad-sense' ),
		...
		if ( $this->field['options']['color'] !== false ) {
			...
			echo '<div class="bg-opt-input-label">'.__( 'Background Color:', 'ad-sense' ).'</div>';
		...
		// Tabs
		echo '<div class="bg-opt-input-label">'.__( 'Background Image:', 'ad-sense' ).'</div>';
		echo '<fieldset class="green buttonset buttonset-tabs ui-buttonset">';
			if ( $this->field['options']['image_pattern'] !== false ) {
				...
				echo '<label id="nhp-opts-button" for="'.$this->field['id'].'_pattern" class="buttonset-tab ui-button ui-state-default ui-button-text-only"><span class="ui-button-text">'.__('Pattern', 'ad-sense' ).'</span></label>';
			}
			if ( $this->field['options']['image_upload'] !== false ) {
				...
				echo '<label id="nhp-opts-button" for="'.$this->field['id'].'_upload" class="buttonset-tab ui-button ui-state-default ui-button-text-only"><span class="ui-button-text">'.__('Upload', 'ad-sense' ).'</span></label>';
			}
			if ( $this->field['options']['gradient'] !== false ) {
				...
				echo '<label id="nhp-opts-button" for="'.$this->field['id'].'_gradient" class="buttonset-tab ui-button ui-state-default ui-button-text-only"><span class="ui-button-text">'.__('Gradient', 'ad-sense' ).'</span></label>';
			}
		...
		if ( $this->field['options']['image_upload'] !== false ) {
			...
			echo ' <a href="javascript:void(0);" class="nhp-opts-upload button-secondary"'.$upload.' rel-id="'.$this->field['id'].'_image_upload">'.__('Browse', 'ad-sense' ).'</a>';
			echo ' <a href="javascript:void(0);" class="nhp-opts-upload-remove"'.$remove.' rel-id="'.$this->field['id'].'_image_upload">'.__('Remove Upload', 'ad-sense' ).'</a>';
			echo '</fieldset>';
			...
			if ( $this->field['options']['repeat'] !== false ) {
				...
				echo '<label for="'.$this->field['id'].'_repeat" class="bg-opt-input-label">'.__( 'Background Repeat:', 'ad-sense' ).'</label>';
			...
			if ( $this->field['options']['attachment'] !== false ) {
				...
				echo '<div class="bg-upload-select">';
				echo '<label for="'.$this->field['id'].'_attachment" class="bg-opt-input-label">'.__( 'Background Attachment:', 'ad-sense' ).'</label>';
			...
			if ( $this->field['options']['position'] !== false ) {
				...
				echo '<div class="bg-upload-select">';
				echo '<label for="'.$this->field['id'].'_position" class="bg-opt-input-label">'.__( 'Background Position:', 'ad-sense' ).'</label>';
			...
			if ( $this->field['options']['size'] !== false ) {
				...
				echo '<div class="bg-upload-select">';
				echo '<label for="'.$this->field['id'].'_size" class="bg-opt-input-label">'.__( 'Background Size:', 'ad-sense' ).'</label>';
		...
		if ( $this->field['options']['gradient'] !== false ) {
			...
			echo '<div class="bg-opt-input-label">' . __('From:', 'ad-sense' ) . '</div><input type="text" id="'.$this->field['id'].'_gradient_from" name="'.$this->args['opt_name'].'['.$this->field['id'].'][gradient][from]" value="'.$this->value['gradient']['from'].'" class="popup-colorpicker" data-default-color="'.$defaults['gradient']['from'].'" style="width:70px;"/>';
			echo '</div>';
			echo '<div class="color-gradient-step-wrapper">';
			echo '<div class="bg-opt-input-label">' . __('To:', 'ad-sense' ) . '</div><input type="text" id="'.$this->field['id'].'_gradient_to" name="'.$this->args['opt_name'].'['.$this->field['id'].'][gradient][to]" value="'.$this->value['gradient']['to'].'" class="popup-colorpicker" data-default-color="'.$defaults['gradient']['to'].'" style="width:70px;"/>';
			echo '</div>';

			echo '<div class="color-gradient-direction-wrapper">';
			echo '<label for="'.$this->field['id'].'_gradient_direction" class="bg-opt-input-label">' . __('Direction:', 'ad-sense' ) . '</label>';
			echo '<select id="'.$this->field['id'].'_gradient_direction" name="'.$this->args['opt_name'].'['.$this->field['id'].'][gradient][direction]" class="color-gradient-direction">';
				echo '<option value="horizontal"' . selected( $this->value['gradient']['direction'], 'horizontal', false ) . '>' . __( 'Horizontal', 'ad-sense' ) . '</option>';
				echo '<option value="vertical"' . selected( $this->value['gradient']['direction'], 'vertical', false ) . '>' . __( 'Vertical', 'ad-sense' ) . '</option>';
			echo '</select>';
			echo '</div>';
			echo '</div>';
			echo '</div>';
		...
		if ( $this->field['options']['parallax'] !== false ) {
			...
			echo '<div class="bg-opt-input-label">'.__( 'Parallax Effect:', 'ad-sense' ).'</div>';
